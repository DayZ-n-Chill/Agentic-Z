...
Set up a DayZ test server instance under `.server/<instance>/`. Copies the mission template from DayZ Server install if missing, creates per-instance `serverDZ.cfg`, `server-profiles/`, `client-profiles/`. Each instance is fully isolated so you can run multiple variants of the same map (`chernarus`, `chernarus-hardcore`, etc.) without cross-contamination. Refuses if the legacy `workspace/_server/` layout exists; run `/dayz-migrate-server` first. Required before `/dayz-launch-test` for a given instance. Use `--refresh-mission` to re-copy mission content after a DayZ update.
