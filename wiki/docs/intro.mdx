...
| [`/dayz-search-index`](./skills/dayz-search-index) | Build the semantic-search index over vanilla DayZ source (powers the `dayz-rag` MCP server) |
| [`/dayz-search-wiki-index`](./skills/dayz-search-wiki-index) | Index the Bohemia community wiki into the same DB |
| [`/dayz-search-download`](./skills/dayz-search-download) | Pull prebuilt vector index from GitHub releases instead of building locally |
...
| [`/docs-sync`](./skills/docs-sync) | Detect drift between canonical sources and the Docusaurus wiki; `--apply` writes the updated mirrors |
