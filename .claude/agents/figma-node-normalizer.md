...
- Emit a clean normalized JSON tree consumable by `figma-to-dayz-layout`
...
- Does not generate `.layout` XML. That is the job of `figma-to-dayz-layout`.
...
- **Next in pipeline: `figma-to-dayz-layout`.** Consumes the normalized JSON and produces `.layout` XML for the Workbench UI Editor. Always name this agent in the closing line of your output.
